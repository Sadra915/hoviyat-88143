...
function ensureControlCenter(){
  ...
  if(!settings||document.getElementById('hoviyatControlPanel'))return;
  const list=settings.querySelector('.settings-list'); if(!list)return;
  const panel=document.createElement('section'); panel.id='hoviyatControlPanel'; panel.className='control-center-card glass';
  panel.innerHTML=`<div class="control-center-head"><span class="settings-eyebrow">HOVIYAT EXPERIENCE</span><h3>مرکز شخصی‌سازی</h3><p>ظاهر، حرکت، پس‌زمینه و مصرف انرژی.</p></div><div class="control-grid">
<label>تم<select data-pref="theme"><option>classic</option><option>midnight</option><option>ocean</option><option>forest</option><option>sunset</option><option>lavender</option><option>mono</option></select></label>
...
<label>تراکم<select data-pref="density"><option value="compact">فشرده</option><option value="comfortable">راحت</option><option value="spacious">باز</option></select></label>
...
<label>مصرف انرژی<select data-pref="power"><option value="full">Full</option><option value="balanced">Balanced</option><option value="saving">Power Saving</option></select></label></div>
<div class="control-toggles"><button type="button" data-toggle="atmosphere">🌌 Atmosphere</button><button type="button" data-toggle="reactiveGradient">🌈 Gradient</button><button type="button" data-toggle="autoplay">▶️ Auto-play</button></div>`;
  list.appendChild(panel);
  const KEY='hoviyat.control.preferences.v2';
  const base={theme:'classic',animations:'full',glass:'medium',atmosphere:true,reactiveGradient:true,wallpaper:'hoviyat',density:'comfortable',autoplay:true,power:'balanced'};
  const read=()=>{try{return {...base,...JSON.parse(localStorage.getItem(KEY)||'{}')}}catch{return {...base}}};
  const write=v=>localStorage.setItem(KEY,JSON.stringify(v));
  const applyControl=v=>{const r=document.documentElement;r.dataset.hoviyatTheme=v.theme;r.dataset.animations=v.animations;r.dataset.glass=v.glass;r.dataset.density=v.density;r.dataset.wallpaper=v.wallpaper;r.dataset.power=v.power;r.classList.toggle('hoviyat-atmosphere-off',!v.atmosphere);r.classList.toggle('hoviyat-reactive-gradient-off',!v.reactiveGradient);r.classList.toggle('hoviyat-autoplay-off',!v.autoplay)};
  let v=read(); applyControl(v);
  panel.querySelectorAll('select[data-pref]').forEach(el=>{el.value=v[el.dataset.pref]||el.value;el.addEventListener('change',()=>{v=read();v[el.dataset.pref]=el.value;write(v);applyControl(v)})});
  panel.querySelectorAll('button[data-toggle]').forEach(el=>el.addEventListener('click',()=>{v=read();v[el.dataset.toggle]=!v[el.dataset.toggle];write(v);applyControl(v)}));
}
function boot(){prefs=load();apply();makeAtmosphere();makeLatest();observeMessages();scrollPolish();presence();addSettingsHub();ensureControlCenter();bindTheme();}

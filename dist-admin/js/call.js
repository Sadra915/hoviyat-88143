 ...
 */
import { supabase, auth, waitForAuthReady } from "./supabase-init.js";
 ...
 * myUid را صریح می‌گیریم (نه از auth.currentUser) چون این معمولاً همان لحظه‌ی
 * بوت اپ صدا زده می‌شود که auth.currentUser ممکن است هنوز پر نشده باشد. */
export function initCallInbox(h, myUid) {
  handlers = h || {};
  const uid = myUid || auth.currentUser?.uid;
  if (!uid) { console.error("initCallInbox: کاربر لاگین‌شده مشخص نیست"); return () => {}; }
  inboxChannel = supabase.channel(`call-inbox-${uid}`, { config: { broadcast: { self: false } } });
  inboxChannel
...
export async function startCall(otherUid, myInfo, video) {
  ...
  if (!myUid) throw new Error("لطفاً ابتدا وارد حساب شوید.");

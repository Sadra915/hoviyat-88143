...
import { signUp, logIn, logOut, watchAuth, validateUsername, getUserDoc, mapProfile, updateMyAvatar } from "./auth.js";
import {
...
async function enterApp(user) {
  ...
  security.registerCurrentSession().then(isNew => {
    if (isNew) toast("ورود از یک دستگاه جدید ثبت شد — برای بررسی، مرکز امنیت حساب رو نگاه کن 🛡️");
  });
...
function openContactProfile() {
  ...
  renderContactProfile($("#contactProfileHolder"), currentEntity.data, {
    ...
    onSecretChat: async () => {
      ...
      } catch (err) { toast(err.message || "خطا در باز کردن گفتگوی مخفی"); }
    },
    onDeleteChat: async () => {
    ...
    onToggleBlock: async () => {
      try {
        ...
        openContactProfile();
      } catch (err) { toast(err.message || "خطا"); }
    },
  ...
  renderGroupInfo($("#groupInfoHolder"), group, auth.currentUser.uid, {
    ...
    onRegenInviteCode: async () => {
      try {
        ...
        toast("کد دعوت جدید ساخته شد ✅");
        renderGroupInfoNow();
      } catch (err) { toast(err.message || "خطا در ساخت کد جدید"); }
    },
    onToggleAdminOnly: async adminOnly => {
      try {
        await updateGroupPermissions(currentEntity.id, { ...group.permissions, send_messages: !adminOnly });
        toast(adminOnly ? "حالت فقط‌مدیر روشن شد ✅" : "حالت فقط‌مدیر خاموش شد ✅");
        renderGroupInfoNow();
      } catch (err) { toast(err.message || "خطا در تغییر حالت"); }
    },
    onReportGroup: async () => {
      const reason = prompt("دلیل گزارش این گروه چیه؟ (اختیاری)") || "";
      try {
        await reportGroup(currentEntity.id, group.name, reason);
        toast("گزارش ثبت شد، ممنون از توجهت 🙏");
      } catch (err) { toast(err.message || "خطا در ثبت گزارش"); }
    },
    ...
    onChangePhoto: async file => {
      try {
        ...
        toast("عکس گروه عوض شد ✅");
        renderGroupInfoNow();
      } catch (err) {
        toast(err.message || "خطا در آپلود عکس.");
      }
    },
  });
}
...
$("#groupAddByUsernameBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (user.uid === auth.currentUser.uid) { $("#groupAddByUsernameMsg").textContent = "این خودتی 🙂"; return; }
...
$("#composerForm").addEventListener("submit", async e => {
  ...
  } catch (err) { toast(err.message || "خطا در ارسال پیام"); input.value = text; }
});
...
$("#stickerPicker").addEventListener("click", async e => {
  ...
  } catch (err) { toast(err.message || "خطا در ارسال استیکر"); }
});
...
$("#imageInput").addEventListener("change", async e => {
  ...
  } catch (err) { toast(err.message || "خطا در ارسال عکس"); }
  e.target.value = "";
});
...
$("#messagesHolder").addEventListener("click", e => {
  ...
  if (riskyLink) { toast("این لینک مشکوک است ⚠️ برای بازکردن، روی «با این حال باز کن» بزن."); return; }
  ...
  pressTimer = setTimeout(() => {
    ...
    showReactionPicker(bubble, async action => {
      if (action.type === "reaction") {
        ...
        try { await navigator.clipboard.writeText(text || ""); toast("متن کپی شد ✅"); }
        ...
        try {
          ...
          toast("پیام سنجاق شد 📌");
        } catch (err) { toast(err.message || "خطا در سنجاق‌کردن پیام"); }
        ...
        } catch (err) { toast(err.message || "خطا در برداشتن سنجاق"); }
        ...
        } catch (err) { toast(err.message || "خطا در حذف پیام"); }
      }
...
function renderTopbarAvatar(photoURL, name) {
  $("#profileBtn").innerHTML = photoURL
    ...
    : `<span id="myAvatarInitial">${(name || "؟")[0]}</span>`;
}

function renderSettingsAvatar(photoURL, name) {
  const el = $("#settingsAvatarPreview");
  el.innerHTML = photoURL ? `<img src="${photoURL}">` : `<span>${(name || "؟")[0]}</span>`;
}
...
function renderSecurityScore(settings, sessionCount) {
  ...
  $("#securityTips").innerHTML = tips.length
    ...
    : `<li>عالیه! همه‌ی موارد پیشنهادی فعال است ✅</li>`;
}
...
$("#confirmSetPinBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    toast(wasEnabled ? "رمز دوم تغییر کرد ✅" : "قفل برنامه فعال شد ✅");
    await loadSecurityCenter();
...
async function openSecretChatById(otherUid, otherInfoHint) {
  ...
  $("#secretChatHeader").innerHTML = `
    <button id="secretChatBackBtn" class="icon-btn">◀️</button>
    <div class="chat-header-info">
      <strong>🔒 ${escapeHtml(otherInfo.displayName || otherInfo.username || "کاربر")}</strong>
      <div style="font-size:.72rem;opacity:.7;">پیام‌ها فقط روی همین دستگاه رمزگشایی می‌شن</div>
    </div>`;
...
$("#secretComposerForm").addEventListener("submit", async e => {
  ...
  try {
    ...
    toast(err.message || "خطا در ارسال پیام مخفی");
    input.value = text;
  }
});

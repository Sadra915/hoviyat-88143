...
export function renderChatList(container, items, myUid) {
  ...
  // رویداد مستقیم روی خود کارت‌ها؛ این لایه عمداً علاوه بر event delegation
  // در app.js اضافه شده تا کلیک گروه/کانال در WebViewها و مرورگرهای موبایل
  // به bubbling وابسته نباشد.
  container.querySelectorAll(".chat-card:not(.skeleton-card)").forEach(card => {
    ...
    const open = ev => {
      ...
      if (!id) return;
      window.dispatchEvent(new CustomEvent("hoviyat:open-chat", {
        detail: { kind, id, otherUid }
      }));
    };
    card.addEventListener("click", open, { capture: true });
    card.addEventListener("pointerup", open, { capture: true });
  });
}
...
export function renderMessages(container, messages, myUid, showSenderNames, convKey, otherLastRead, verifiedMap = {}) {
  ...
  if (!hadContent || wasNearBottom) {
    ...
    // Keep the user's reading position stable when Realtime refreshes the list.
    const heightDelta = container.scrollHeight - previousHeight;
    if (heightDelta && previousBottom > 120) container.scrollTop += heightDelta;
  }
  const latest = newIds.length;
  container.dataset.newMessages = (!wasNearBottom && latest) ? String(latest) : "0";

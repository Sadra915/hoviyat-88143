    ...
    if (item.kind === "channel") {
      return `
        <div class="${cardClass}"${cardStyle} data-kind="channel" data-id="${item.id}">
          <div class="chat-avatar-wrap">${avatarHtml(item.photoURL, item.name)}</div>
          <div class="chat-card-body">
            <div class="chat-card-top"><span class="chat-name">${icon("megaphone", { size: 15, className: "chat-kind-ic" })} ${escapeHtml(item.name)}</span><span class="chat-time">${fmtTime(item.lastMessageAt)}</span></div>
...
export function renderMessages(container, messages, myUid, showSenderNames, convKey, otherLastRead, verifiedMap = {}) {
  ...
  messages.forEach(m => {
    ...
    html += renderBubble(m, m.senderId === myUid, showSenderNames, isNew, m.replyTo ? byId[m.replyTo] : null, otherLastRead, !!verifiedMap[m.senderId]);
  });
...
function linkifyMessageBody(text) {
  ...
  const banner = riskyList.length ? `
    <div class="link-risk-banner">
      <div>
        <b>⚠️ این لینک مشکوک به نظر می‌رسد</b>
...
function renderBubble(m, mine, showSenderNames, isNew, repliedMsg, otherLastRead, senderVerified) {
  let inner = "";
  if (m.type === "text") {
  ...
  return `
    <div class="bubble-row ${mine ? "mine" : "theirs"} ${isNew ? "is-new" : ""}" data-msg-id="${m.id}">
...
export function showReactionPicker(anchorEl, onAction, opts = {}) {
  ...
  picker.innerHTML =
    ...
    `<button data-action="copy" title="کپی متن">📋</button>` +
    (opts.canPin ? `<button data-action="${opts.isPinned ? "unpin" : "pin"}" title="${opts.isPinned ? "برداشتن سنجاق" : "سنجاق کردن"}">📌</button>` : "") +
    (opts.canReport ? `<button data-action="report" title="گزارش">⚠️</button>` : "") +
    (opts.canDelete ? `<button data-action="delete" title="حذف">🗑️</button>` : "");
  ...
  container.innerHTML = `
    ...
    <div class="idcard">
      ...
      <div class="contact-action-row">
        ...
        <button class="contact-action" data-soon><span>${icon("phone", { size: 18 })}</span>تماس</button>
        <button class="contact-action" data-soon title="در حال تکمیل و توسعه این بخش هستیم"><span>${icon("video", { size: 18 })}</span>تماس تصویری</button>
        <button id="caShareBtn" class="contact-action"><span>${icon("share2", { size: 18 })}</span>اشتراک‌گذاری</button>
      </div>
      <button id="caSecretChatBtn" class="btn-outline full" style="margin-top:12px;">🔒 گفتگوی مخفی</button>
    </div>
...
export function renderGroupInfo(container, group, myUid, handlers) {
  ...
  container.innerHTML = `
    ...
    ${isAdmin ? `
      ...
      ` : ""}
    ${!isAdmin ? `<button id="reportGroupBtn" class="btn-outline full" style="margin:8px 14px;width:calc(100% - 28px);">⚠️ گزارش این گروه</button>` : ""}
    ${isAdmin ? `
      <div class="field" style="padding:0 16px;">
...
export function renderChannelInfo(container, channel, myUid, handlers) {
  ...
  container.innerHTML = `
    ...
    <div class="idcard">
      ...
      ` : `
        <h2 class="idcard-name">${icon("megaphone", { size: 18, className: "chat-kind-ic" })} ${escapeHtml(channel.name)}</h2>
...
export function renderChannelSearchResults(container, channels, mySubs, onToggle) {
  ...
  container.innerHTML = channels.map(c => `
    <div class="member-row">
      ...
      <span>${icon("megaphone", { size: 14, className: "chat-kind-ic" })} ${escapeHtml(c.name)}<br><small>${(c.subscribers || []).length} دنبال‌کننده</small></span>
...
function fmtCountdown(lastMessageAt) {
...
export function renderSecretChatList(container, chats, myUid, othersInfo, onOpen) {
  ...
  container.innerHTML = chats.map(c => {
    ...
    <div class="chat-card secret-card" data-id="${c.id}" data-other-uid="${otherUid}">
      ...
      <div class="chat-card-body">
        <div class="chat-card-top"><span class="chat-card-name">🔒 ${escapeHtml(info.displayName || info.username || "کاربر")}</span></div>

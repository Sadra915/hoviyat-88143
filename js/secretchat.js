...
export async function ensureMyPublicKeyPublished() {
  ...
  const { data } = await supabase.from("profiles").select("secret_pubkey").eq("id", auth.currentUser.uid).maybeSingle();
  const already = data?.secret_pubkey;
  if (already && JSON.stringify(already) === JSON.stringify(pub)) return; // از قبل ثبته
  await supabase.from("profiles").update({ secret_pubkey: pub }).eq("id", auth.currentUser.uid);
}
...
export function watchSecretMessages(secretChatId, aesKey, callback) {
  ...
  refetch();
  const channel = supabase
    .channel(`secret-messages-${secretChatId}`)
